...
def utility_dashboard(request):
    ...
    avg_time = LeaderboardEntry.objects.aggregate(Avg('session_duration_seconds'))['session_duration_seconds__avg'] or 0
    avg_questions = LeaderboardEntry.objects.aggregate(Avg('total_questions_answered'))['total_questions_answered__avg'] or 0
    avg_accuracy = LeaderboardEntry.objects.aggregate(Avg('session_accuracy_percent'))['session_accuracy_percent__avg'] or 0
    avg_mastery_gain = LeaderboardEntry.objects.aggregate(Avg('mastery_gained'))['mastery_gained__avg'] or 0

    # 2. Extract Raw Telemetry for the Frontend Graphs
    # We grab just the fields we need to keep the payload lightning fast
    raw_telemetry = list(LeaderboardEntry.objects.values(
        'timestamp', 'age_bracket', 'country', 'state',
        'session_duration_seconds', 'total_questions_answered',
        'mastery_gained', 'levels_gained'
    ))
